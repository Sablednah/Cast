#!/usr/bin/env bash
# Build Cast and copy the jar into a CurseForge instance.
#
# The target instance is chosen from the jar's Minecraft version, so it follows
# whichever version branch is checked out:
#
#   cast-0.1.0+mc1.21.11.jar  -> the 1.21.11 instance (default below)
#   cast-0.1.0+mc26.1.2.jar   -> the "26.1.2" instance
#   cast-0.1.0+mc26.2.jar     -> the "26.2" instance
#
# Usage:  ./deploy.sh
#         CAST_INSTANCE="/path/to/instance" ./deploy.sh    # override the target
set -euo pipefail
...
# Every instance on this Minecraft line that already carries Cast gets the jar too (a "26.2" and a
...
TARGETS=()
if [ -n "${CAST_INSTANCE:-}" ]; then
    TARGETS+=("$CAST_INSTANCE")
else
    TARGETS+=("$TARGET")
    WANT="${MC_TAG:-1.21.11}"
    for d in "$INSTANCES"/*/; do
        d="${d%/}"
        [ "$d" = "$TARGET" ] && continue
        ls "$d/mods"/cast-*.jar >/dev/null 2>&1 || continue
...
for INSTANCE in "${TARGETS[@]}"; do
    ...
    echo ">> Removing previous Cast jars from '$NAME'..."
    rm -f "$MODS"/cast-*.jar
    cp "$JAR" "$MODS/"
